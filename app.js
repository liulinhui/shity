...
let appConfig = program.config ? require(path.resolve(process.cwd(), program.config)) : require('./config.json');

let genesisblock = program.genesis ? require(path.resolve(process.cwd(), program.genesis)) : require('./genesisBlock.json');

let networks = program.networks ? require(path.resolve(process.cwd(), program.networks)) : require('./networks.json');
...
if (appConfig.network) {
    ...
    appConfig.network = networks.shity;
}
...
d.run(function () {
    ...
    async.auto({
        ...
        connect: ['config', 'genesisblock', 'logger', 'build', 'network', function (scope, cb) {
            ...
            let requestSanitizer = require('./helpers/request-sanitizer');
            ...
            scope.network.server.listen(scope.config.port, scope.config.address, function (err) {
                scope.logger.info('# Shity node server started on: ' + scope.config.address + ':' + scope.config.port);

                if (!err) {
                    if (scope.config.ssl.enabled) {
                        scope.network.https.listen(scope.config.ssl.options.port, scope.config.ssl.options.address, function (err) {
                            scope.logger.info('Shity https started: ' + scope.config.ssl.options.address + ':' + scope.config.ssl.options.port);
        ...
        crypto: ['config', function (scope, cb) {
            let Crypto = require('./helpers/crypto.js');
...
function startInteractiveMode(scope) {
    ...
    vorpal
        ...
        .action(function (args, callback) {
            ...
            scope.modules.loader.getNetwork(true, function (err, network) {
                let lastBlock = scope.modules.blockchain.getLastBlock();
            ...
            let peers = scope.modules.peers.listBroadcastPeers();
    ...
    vorpal
        ...
        .action(function (args, callback) {
            let self = this;
            let grep = spawn('grep', ['-e', args.query, appConfig.logFileName]);
    ...
    vorpal
        ...
        .action(function (args, callback) {
            ...
            scope.db.query(args.query).then(function (rows) {
                self.log(rows.map(function (row) {
    ...
    vorpal
        ...
        .action(function (args, callback) {
            let self = this;
            let crypto=require('shity-js').crypto;
            let passphrase = require("bip39").generateMnemonic();
            self.log("Seed    - private:", passphrase);
            self.log("WIF     - private:", crypto.getKeys(passphrase).toWIF());
            self.log("Address - public :", crypto.getAddress(crypto.getKeys(passphrase).publicKey));
            callback();
        });
    let account = null;
    vorpal
    ...
    vorpal
        ...
        .action(function (args, callback) {
            scope.db.query("select * from mem_accounts where address ='" + args.address + "'").then(function (rows) {
                let publicKey = rows[0].publicKey.toString("hex");
                let receivedSQL = 'select sum(amount) as total, count(amount) as count from transactions where amount > 0 and "recipientId" = \'' + args.address + '\';'
    ...
    vorpal.history('shity-node');

    vorpal
        .delimiter('shity-node>')
        .show();
}
