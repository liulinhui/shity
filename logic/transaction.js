...
// Private fields
let self, __private = {}, genesisblock = null;
...
Transaction.prototype.sign = function (keypair, trs) {
    return this.scope.crypto.sign(this.getHash(trs), keypair).toString('hex');
};
...
Transaction.prototype.getBytes = function (trs, skipSignature, skipSecondSignature) {
    ...
    try {
        ...
        if (trs.recipientId) {
            let recipient = bs58check.decode(trs.recipientId);
        ...
        if (trs.vendorField) {
            let vf = new Buffer(trs.vendorField);
...
Transaction.prototype.checkBalance = function (amount, balance, trs, sender) {
    let exceededBalance = bignum(sender[balance].toString()).lessThan(amount);
    let exceeded = (trs.blockId !== genesisblock.block.id && exceededBalance);
...
Transaction.prototype.verify = function (trs, sender, requester, cb) {
    ...
    let amount = bignum(trs.amount.toString()).plus(trs.fee.toString());
...
Transaction.prototype.verifyFee = function (trs) {
    ...
    let fee = __private.types[trs.type].calculateFee.call(this, trs);
...
Transaction.prototype.verifyBytes = function (bytes, publicKey, signature) {
    ...
    try {
        let data2 = new Buffer(bytes.length);
...
Transaction.prototype.undo = function (trs, block, sender, cb) {
    let amount = bignum(trs.amount.toString());
...
Transaction.prototype.dbSave = function (trs) {
    ...
    let senderPublicKey, signature, signSignature, requesterPublicKey, vendorField;
