...
BlockReward.prototype.calcMilestone = function (height) {
    let location = Math.trunc((__private.parseHeight(height) - this.rewardOffset) / this.distance);
...
BlockReward.prototype.calcSupply = function (height) {
    ...
    let supply = constants.totalAmount / Math.pow(10, 8);
    let rewards = [];
        ...
        if (milestone >= i) {
            ...
            if (height < this.rewardOffset) {
                ...
                if (height > 0 && i === this.milestones.length - 1) {
                    let postHeight = this.rewardOffset - 1;
