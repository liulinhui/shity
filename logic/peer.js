...
const __schemas = require('../schema/api.peer.js');
...
Peer.prototype.request = function (api, options, cb) {
    ...
    let request = popsicle.request(req);
    this.lastchecked = new Date().getTime();
    let that = this;
    request.use(popsicle.plugins.parse(['json'], false)).then(function (res) {
        ...
        if (res.status !== 200) {
            ...
            let apihandle = api.split("?")[0];
            let report = true;
            let apischema = options.schema || __schemas[options.method + ":" + apihandle];
            if (apischema) {
            ...
            let header = that.normalizeHeader(res.headers);
