...
// Private fields
// TODO: to be removed
let modules, library, self, __private = {}, genesisblock = null, shared = {};
...
shared.getAccounts = function (req, cb) {
    library.schema.validate(req.body, schema.getAccounts, function (err) {
        ...
        library.db.one(sql.getAccounts, {publicKey: req.body.publicKey}).then(function (row) {
            let addresses = Array.isArray(row.accountId) ? row.accountId : [];
        ...
        buildTransactions: function (seriesCb) {
            async.eachSeries(scope.transactions, function (transaction, eachSeriesCb) {
                ...
                modules.accounts.getAccount({publicKey: transaction.senderPublicKey}, function (err, sender) {
                    ...
                    let min = transaction.asset.multisignature.min;
                    let lifetime = transaction.asset.multisignature.lifetime;
                    let signatures = transaction.asset.multisignature.keysgroup;
...
shared.sign = function (req, cb) {
    ...
    library.balancesSequence.add(function (cb) {
        async.series({
            ...
            let transaction = modules.transactionPool.getMultisignatureTransaction(req.body.transactionId);
...
Multisignatures.prototype.processSignature = function (tx, cb) {
    let transaction = modules.transactionPool.getUnconfirmedTransaction(tx.transaction);
