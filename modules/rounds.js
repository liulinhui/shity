        ...
        if (err) {
            ...
            modules.accounts.mergeAccountAndGet({
                ...
                if (err) {
                    ...
                    __private.collectedfees[round] -= block.totalFee;
                    let generator = __private.forgers[round].pop();
...
__private.changeRoundForward = function (block, cb) {
    let nextround = __private.current + 1;
        ...
        if (err) {
            ...
            __private.generateDelegateList(nextround, function (err, fullactivedelegates) {
                if (err) {
                    ...
                    __private.activedelegates[nextround] = fullactivedelegates.map(function (ad) {
                        return ad.publicKey
                    });
...
__private.checkAndChangeRoundBackward = function (block, cb) {
    let round = __private.current;
    let blockround = self.getRoundFromHeight(block.height);
...
__private.updateActiveDelegatesStats = function (cb) {
    let round = __private.current;
    let activedelegates = __private.activedelegates[round];
    let forgers = __private.forgers[round];
    let forgerStats = {};

    for (let i in forgers) {
...
__private.randomizeDelegateList = function (activedelegates, round) {
    ...
    for (let i = 0, delCount = activedelegates.length; i < delCount; i++) {
        for (let x = 0; x < 4 && i < delCount; i++, x++) {
            ...
            let b = activedelegates[newIndex];
...
__private.getCurrentRoundForgers = function (cb) {
    ...
    let firstHeightOfround = (round - 1) * slots.delegates;

    library.db.query(sql.getRoundForgers, {
        minheight: firstHeightOfround,
        maxheight: lastBlock.height
    }).then(function (rows) {
        return cb(null, rows);
    }).catch(cb);
    ...
    if (__private.activedelegates[round]) {
        ...
        library.db.query(sql.getActiveDelegates, {round: round}).then(function (rows) {
            if (rows.length === constants.activeDelegates) {
                ...
                __private.activedelegates[round] = rows.map(function (row) {
            ...
            // ok maybe we just started node from scratch, so need to generate it.
                ...
                __private.generateDelegateList(round, function (err, activedelegates) {
                    ...
                    __private.activedelegates[round] = activedelegates.map(function (ad) {
                        return ad.publicKey;
                    });
...
Rounds.prototype.onDatabaseLoaded = function (lastBlock) {
    ...
    let round = self.getRoundFromHeight(lastBlock.height);
