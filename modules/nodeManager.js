...
const schema = require('../schema/nodeManager.js');
const sql = require('../sql/nodeManager.js');
const os = require('os');

let self, library, modules;
...
NodeManager.prototype.onDelegatesLoaded = function (keypairs) {
    let numberOfDelegates = Object.keys(keypairs).length;
    let loadedPairs = Object.keys(__private.keypairs).length;
        ...
        self.performSPVFix(function (err, results) {
            ...
            modules.loader.getNetwork(true, function (err, network) {
                ...
                if (!network || !network.height) {
                    ...
                    let bestBlock = modules.loader.getNetworkSmallestBlock();
                    ...
                    if (bestBlock && bestBlock.height > lastBlock.height) {
                        library.logger.info("Observed network is on same height, but some peers with bigger height", {
                            network: {
                                id: bestBlock.id,
                                height: bestBlock.height
                            }, node: {id: lastBlock.id, height: lastBlock.height}
                        });
                        library.logger.info("Rebuilding from network");
                        return modules.blocks.removeSomeBlocks(blocksToRemove, mSequence);
                    }
                    else if (bestBlock && bestBlock.height === lastBlock.height && bestBlock.timestamp < lastBlock.timestamp) {
                        library.logger.info("Observed network is on same height, but found a block with smaller timestamp", {
                            network: {
                                id: bestBlock.id,
                                height: bestBlock.height
                            }, node: {id: lastBlock.id, height: lastBlock.height}
                        ...
                        library.logger.info("Observed network is on same height, and same block timestamp", {
                            network: network.height,
                            node: lastBlock.height
                        });
...
NodeManager.prototype.performSPVFix = function (cb) {
    ...
    library.db.query('select address, "publicKey", balance from mem_accounts').then(function (rows) {
        async.eachSeries(rows, function (row, eachCb) {
            ...
            let receivedSQL = 'select sum(amount) as total, count(amount) as count from transactions where amount > 0 and "recipientId" = \'' + row.address + '\';'
            let spentSQL = 'select sum(amount+fee) as total, count(amount) as count from transactions where "senderPublicKey" = \'\\x' + publicKey + '\';'
            let rewardsSQL = 'select sum(reward+"totalFee") as total, count(reward) as count from blocks where "generatorPublicKey" = \'\\x' + publicKey + '\';'
            ...
            async.series(series, function (err, result) {
                ...
                if (result.balance !== row.balance) {
                    ...
                    let diff = result.balance - row.balance;
...
__private.prepareBlock = function (block, peer, cb) {
    ...
    // carefully since order is important to validate block
        ...
        let transactionIds = block.transactionIds;
        ...
        modules.transactionPool.getMissingTransactions(transactionIds, function (err, missingTransactionIds, foundTransactions) {
            ...
            // lets download the missing ones from the peer that sent the block.
                ...
                modules.transport.requestFromPeer(peer, {
                        ...
                        let receivedTransactions = res.body.transactions;
                        ...
                        for (let i = 0; i < transactionIds.length; i++) {
                            ...
                            let tx = receivedTransactions.find(function (tx) {
                                return tx ? tx.id === id : false
                            });
                            if (tx) {
                                transactionIds[i] = tx;
                                ...
                                tx = foundTransactions.find(function (tx) {
                                    return tx.id === id
                                });
                                if (!tx.incomplete) {
                                    transactionIds[i] = tx;
...
NodeManager.prototype.swapLastBlockWith = function (block, peer, cb) {
    async.series([
        function (seriesCb) {
            let check = modules.blocks.verifyBlockHeader(block);
        ...
        if (!block.ready) {
            if (block.orphaned) {
                ...
                if (lastBlock.height > block.height) {
                    library.logger.info("Orphaned block arrived over one block time too late, block disregarded", {
                        id: block.id,
                        height: block.height,
                ...
                if (lastBlock && block.timestamp < lastBlock.timestamp) {
                    ...
                    library.logger.info("Orphaned block has a smaller timestamp, swaping with lastBlock", {
                        id: block.id,
                        height: block.height
                    });
                    ...
                    library.logger.info("Orphaned block has same timestamp but smaller id, swaping with lastBlock", {
                        id: block.id,
                        height: block.height
                    });
                    return self.swapLastBlockWith(block, peer, mSequence);
                    ...
                    // no swap
                    library.logger.info("Orphaned block has a bigger timestamp or bigger id, block disregarded", {
                        id: block.id,
                        height: block.height
                    });
                    return mSequence(null, block);
            ...
            library.logger.info("New block received", {
                id: block.id,
                height: block.height,
...
NodeManager.prototype.onTransactionsReceived = function (transactions, source, cb) {
    library.managementSequence.add(function (mSequence) {
        ...
        // we need sanity check of the transaction list
            ...
            let report = library.schema.validate(transactions, schema.transactions);

            if (!report) {
                return mSequence && mSequence("Transactions list is not conform", transactions);
            }

            let skimmedtransactions = [];
