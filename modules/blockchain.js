...
Blockchain.prototype.onStartBlockchain = function () {
    setImmediate(function listenBlockchainState() {
        let state = __private.timestampState();
        if (state.rebuild) {
            let timedout = false;
    ...
    setImmediate(function cleanBlockchain() {
        ...
        let blockremoved = __private.blockchain[height - 200];
    ...
    if (__private.blockchain[block.height] && __private.blockchain[block.height].id != block.id) {
        if (__private.blockchain[block.height] && __private.blockchain[block.height].generatorPublicKey == block.generatorPublicKey && __private.blockchain[block.height].timestamp == block.timestamp) {
            modules.accounts.getAccount({publicKey: block.generatorPublicKey}, function (err, delegate) {
                library.logger.warn("Double forgery", {
                    id: block.id,
...
Blockchain.prototype.isForked = function (block) {
    let previousBlock = __private.blockchain["" + (block.height - 1)];
...
Blockchain.prototype.isReady = function (block) {
    let ready = __private.lastBlock.height == block.height - 1;
    if (ready) {
        return true;
    }
    else {
...
Blockchain.prototype.getLastVerifiedBlock = function () {
    let lastBlock = null;
    for (let height in __private.blockchain) {
        if (!lastBlock) {
...
Blockchain.prototype.onBlockReceived = function (block, peer) {
    ...
    if (self.isForked(block)) {
        __private.orphanedBlocks[block.id] = block;
        let previousBlock = self.getPreviousBlock(block);
        library.logger.info("Forked block received", {
            id: block.id,
            height: block.height,
            previousBlock: block.previousBlock,
            previousBlockchainBlock: previousBlock.id,
            peer: peer.string
        });
        return;
    }
    ...
    if (!self.isReady(block)) {
        let lastBlock = self.getLastBlock();
        library.logger.info("Blockchain not ready to receive block", {
            id: block.id,
            height: block.height,
...
Blockchain.prototype.onBlockForged = function (block) {
    if (self.isPresent(block)) {
        modules.accounts.getAccount({publicKey: block.generatorPublicKey}, function (err, delegate) {
            library.logger.error("Double forgery - Same block - please disable delegate on one node", {
                id: block.id,
                generator: block.generatorPublicKey,
                username: delegate.username,
                height: block.height
    ...
    if (self.isOrphaned(block)) {
        modules.accounts.getAccount({publicKey: block.generatorPublicKey}, function (err, delegate) {
            library.logger.error("Double forgery - Orphaned block - please disable delegate on one node", {
                id: block.id,
                generator: block.generatorPublicKey,
                username: delegate.username,
                height: block.height
    ...
    if (self.isForked(block)) {
        let previousBlock = self.getPreviousBlock(block);
        modules.accounts.getAccount({publicKey: block.generatorPublicKey}, function (err, delegate) {
            library.logger.error("Double forgery - Forked block - please disable delegate on one node", {
                id: block.id,
                generator: block.generatorPublicKey,
                username: delegate.username,
                height: block.height,
