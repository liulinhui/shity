...
// Private fields
let modules, library, self, __private = {}, shared = {}, genesisblock = null;
...
__private.getPreviousBlock = function (block, cb) {
    let previousBlock = modules.blockchain.getPreviousBlock(block);
    if (previousBlock) {
...
Blocks.prototype.count = function (cb) {
    library.db.query(sql.countByRowId).then(function (rows) {
        let res = rows.length ? rows[0].count : 0;
...
Blocks.prototype.loadBlocksData = function (filter, options, cb) {
    ...
    let fields = __private.blocksDataFields;
...
Blocks.prototype.loadBlocksOffset = function (limit, offset, verify, cb) {
    ...
    library.db.query(sql.loadBlocksOffset, params).then(function (blocks) {
        ...
        //let blocks = __private.readDbRows(rows);
        ...
        async.eachSeries(blocks, function (block, seriesCb) {
            ...
            if (verify && block.id !== genesisblock.block.id) {
                ...
                let check = self.verifyBlock(block, true);
    ...
    async.series({
        ...
        popLastBlock: function (seriesCb) {
            async.whilst(
                ...
                function (next) {
                    let block = modules.blockchain.getLastBlock();
...
Blocks.prototype.getLastBlock = function () {
    ...
    if (lastBlock) {
        let epoch = constants.epochTime / 1000;
        let lastBlockTime = epoch + lastBlock.timestamp;
...
Blocks.prototype.onVerifyBlock = function (block, cb) {
    let result = self.verifyBlock(block, true);
...
Blocks.prototype.verifyBlock = function (block, checkPreviousBlock) {
    ...
    if (previousBlock) {
        let lastBlockSlotNumber = slots.getSlotNumber(previousBlock.timestamp);
    ...
    let calculatedHash = payloadHash.digest().toString('hex');
...
__private.applyBlock = function (block, cb) {
    ...
    let appliedUnconfirmedTransactions = {}, appliedTransactions = {};
    ...
    let removedTransactionsIds, keptTransactions;
...
Blocks.prototype.loadBlocksFromPeer = function (peer, cb) {
    let lastValidBlock = modules.blockchain.getLastBlock();
    ...
    modules.transport.requestFromPeer(peer, {
        ...
        let report = library.schema.validate(res.body.blocks, schema.loadBlocksFromPeer);
...
Blocks.prototype.generateBlock = function (keypair, timestamp, cb) {
    ...
    let transactions = modules.transactionPool.getUnconfirmedTransactionList(false, constants.maxTxsPerBlock);
    let ready = [];
