...
__private.loadUnconfirmedTransactions = function (cb) {
    modules.transport.requestFromRandomPeer({
        ...
        let report = library.schema.validate(res.body, schema.loadUnconfirmedTransactions);

        if (!report) {
            return cb("Transactions list is not conform");
        }

        let transactions = res.body.transactions;
...
__private.loadBlockChain = function () {
    ...
    // 	let missed = !(results[1].count);
    //
    // 	library.logger.info('Blocks ' + count);
    //
    // 	let round = modules.rounds.getRoundFromHeight(count);
    //
    ...
    // 	}
    //
    // 	let unapplied = results[2].filter(function (row) {
...
__private.loadBlocksFromNetwork = function (cb) {
    ...
    let network = __private.network;

    let peers = __private.shuffle(network.peers).sort(function (p1, p2) {
...
Loader.prototype.rebuildBalance = function (cb) {
    let accounts = {};
    let addressesSQL = 'select distinct("recipientId") as address from transactions group by "recipientId"'
    let publicKeysSQL = 'select distinct("senderPublicKey") as publicKey from transactions group by "senderPublicKey"';
    async.series([
        function (seriesCb) {
...
Loader.prototype.getNetwork = function (force, cb) {
    ...
    let distance = modules.delegates.isActiveDelegate() ? 2 : 51;
    ...
    let peers = modules.peers.listPBFTPeers();
