...
TransactionPool.prototype.onStartTransactionPool = function () {
    ...
    setImmediate(function cleanMempool() {
        let expirationdate = slots.getTime() - __private.mempoolConfig.maximumAgeInMinutes * 60;
        let removed = 0;
        let kept = 0;
        for (let txid in __private.mempool) {
...
TransactionPool.prototype.getMergedTransactionList = function (reverse, limit) {
    ...
    let unconfirmed = modules.transactionPool.getUnconfirmedTransactionList(false, constants.maxTxsPerBlock);
    limit -= unconfirmed.length;

    let multisignatures = modules.transactionPool.getMultisignatureTransactionList(false, false, constants.maxTxsPerBlock);
    limit -= multisignatures.length;

    let queued = modules.transactionPool.getQueuedTransactionList(false, limit);
    limit -= queued.length;
...
TransactionPool.prototype.receiveTransactions = function (transactions, cb) {
    ...
    async.eachSeries(transactions, function (transaction, eachSeriesCb) {
        let memtx = __private.mempool[transaction.id];
        if (memtx) {
...
TransactionPool.prototype.undoUnconfirmedList = function (keepUnconfirmedTransactions, cb) {
    let removedIds = [], keptIds = [];
    let keepIds = keepUnconfirmedTransactions.map(function (tx) {
        return tx.id
    });
...
__private.getMissingTransactions = function (ids, cb) {
    let missingtransactionsids = [];
    // copy of ids
    let transactions = JSON.parse(JSON.stringify(ids));
    for (let i in ids) {
        let tx = __private.mempool[ids[i]];
        if (tx) {
...
__private.expireTransactions = function (transactions, parentIds, cb) {
    ...
    async.eachSeries(transactions, function (transaction, eachSeriesCb) {
        ...
        let seconds = Math.floor((timeNow.getTime() - new Date(transaction.receivedAt).getTime()) / 1000);
