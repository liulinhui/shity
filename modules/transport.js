...
function Transport(cb, scope) {
    ...
    setInterval(function () {
        if (__private.broadcastTransactions.length > 0) {
            let transactions = __private.broadcastTransactions;
...
__private.attachApi = function () {
        ...
        req.sanitize(headers, schema.headers, function (err, report) {
            ...
            if (headers.nethash !== library.config.nethash) {
                ...
                return res.status(500).send({
                    ...
                    expected: library.config.nethash,
    ...
    router.get('/list', function (req, res) {
        ...
        let peers = modules.peers.listBroadcastPeers();
        ...
        req.sanitize(req.query, schema.commonBlock, function (err, report, query) {
            ...
            library.db.query(sql.getCommonBlock, escapedIds).then(function (rows) {
                return res.json({
                    success: true,
                    common: rows[0] || null,
                    lastBlockHeight: (lastBlock ? lastBlock.height : 0)
                });
            }).catch(function (err) {
    ...
    // 	res.set(__private.headers);
    //
    // 	let unconfirmedList = modules.transactionPool.getUnconfirmedTransactionList();
    // 	let signatures = [];
    //
...
Transport.prototype.requestFromRandomPeer = function (config, options, cb) {
    ...
    async.retry(20, function (cb) {
        let peers = modules.peers.listGoodPeers();
        if (peers.length) {
...
Transport.prototype.onBroadcastBlock = function (block) {
    ...
    if (block.numberOfTransactions > 0) {//i send only ids, because nodes likely have already transactions in mempool.
        blockheaders.transactionIds = block.transactions.map(function (t) {
            return t.id
        });

    }

    self.broadcast({all: block.forged, limit: limitbroadcast}, {
        api: '/blocks',
        data: {block: blockheaders},
        method: 'POST'
    });
};
